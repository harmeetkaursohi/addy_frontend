...
const Planner = () => {
  ...
  const getPostsForPlannerApi = useGetPostsForPlannerQuery(
    {
      ...
      plannerCardDate: baseSearchQuery.plannerCardDate
        ? baseSearchQuery.plannerCardDate.toISOString()
        : null,
      creationDateRange: {
    ...
    {
      skip:
        isNullOrEmpty(calendarRef) ||
        isNullOrEmpty(baseSearchQuery) ||
  ...
  useEffect(() => {
    if (
      !getPostsForPlannerApi.isLoading &&
      !getPostsForPlannerApi?.isFetching &&
      getPostsForPlannerApi?.data
    ) {
  ...
  const renderCalendarCards = ({ event }) => {
    ...
    const postOnSocialMedia =
      event?._def?.extendedProps?.childCardContent?.length > 0
        ? event?._def?.extendedProps?.childCardContent[0]
        : null;

    return (
      <div
        className={`cal_Div w-100 test`}
        //  style={{
        ...
        <div className="w-100 p-0 calendar_card">
          {postOnSocialMedia !== null && (
            <div className={"custom_event"}>
              <img
                src={postOnSocialMedia?.imageUrl}
                alt={postOnSocialMedia.title}
              />
              <h3 className={`custom_event_heading${classname}`}>
                {postOnSocialMedia.title}
              </h3>
            </div>
          )}
        </div>
  ...
  const customHeaderClick = (eventType) => {
    ...
    setSelectedDate(
      getDayStartInUTC(inst.getDate(), inst.getMonth(), inst.getFullYear())
    );
  ...
  const handleSocialMediaFilters = (curKey) => {
    if (curKey === "all") {
      setBaseSearchQuery((prevSearchQuery) => {
        ...
        return {
          ...prevSearchQuery,
          socialMediaTypes: Object.keys(SocialAccountProvider).every((type) =>
            socialMediaTypes.includes(type)
          )
            ? []
            : Object.keys(SocialAccountProvider),
        };
      });
    } else {
      setBaseSearchQuery((prevSearchQuery) => {
        const updatedSocialMediaTypes = prevSearchQuery.socialMediaTypes
          ? [...prevSearchQuery.socialMediaTypes]
          : [];
          ...
          <div className="cmn_outer">
              ...
              <ul className="schdeuled_post_list">
                {(getPlannerPostsCountApi?.isLoading ||
                  ...
                  <>
                    <li>
                      <h4>
                        <SkeletonEffect count={1} className={"w-25 m-auto "} />
                      </h4>
                      <h3>
                        <SkeletonEffect
                          count={1}
                ...
                {!getPlannerPostsCountApi?.isLoading &&
                  !getPlannerPostsCountApi?.isFetching &&
                  getPlannerPostsCountApi?.data &&
                  Object.keys(getPlannerPostsCountApi.data).map(
                    (key, index) => {
                      return (
                        <li key={index}>
            ...
            >
              <div className="calender_outer_wrapper">
                ...
                <div className={`CalenderOuter_Wrapper`}>
                  <div
                    className={`planner_calender w-100 ${
                      selectedDate && "select_wrapper"
                    }`}
                  >
                      ...
                      <Dropdown.Menu>
                        <li>
                          ...
                          <input
                            ...
                            type={"checkbox"}
                            checked={
                              Array.isArray(baseSearchQuery.socialMediaTypes)
                                ? Object.keys(SocialAccountProvider).every(
                                    (type) =>
                                      baseSearchQuery.socialMediaTypes.includes(
                                        type
                                      )
                                  )
                                : false
                        ...
                        {Object.keys(SocialAccountProvider).map(
                          (curKey, ind) => {
                            return (
                              <li key={ind}>
                                <div className="d-flex gap-2 align-items-center ">
                                  <img
                                    src={computeImageURL(curKey)}
                                    height="20px"
                                    width="20px"
                                  />
                                ...
                                <input
                                  ...
                                  type="checkbox"
                                  checked={
                                    baseSearchQuery.socialMediaTypes &&
                                    baseSearchQuery.socialMediaTypes.includes(
                                      curKey
                                    )
                                  }
                                  value={curKey}
                                  onChange={(e) =>
                                    handleSocialMediaFilters(curKey)
                                  }
                                />
                              </li>
                            );
