...
const CommonPostFields = ({formType, getPostsByBatchIdList = null}) => {
    ...
    useEffect(() => {
        if (formType = "Update" && getPostsByBatchIdList) {
            const {FACEBOOK} = getPostsByBatchIdList;

            console.log("FACEBOOK", FACEBOOK);

            handleSeparateCaptionHashtag(FACEBOOK[0]?.message);

            const pageIds = FACEBOOK.map(data => data.id.split("_")[0]);

            console.log("FACEBOOK pageIds", pageIds);

            // if(socialAccountData!="undefined"){
            //     socialAccountData?.map(el => {
            //         el.pageAccessToken.map(obj => {
            //             console.log("@@@ pageIds.includes(obj.pageId)",pageIds.includes(obj.pageId))
            ...
            if (socialAccountData) {
                const updatedSocialAccountData = socialAccountData.map(el => {
                    ...
                    return {
                        ...
                        selected: updatedPageAccessToken.some(obj => obj.selected)
                    };
                });

                console.log("updatedSocialAccountData", updatedSocialAccountData);
                setSocialAccountData(updatedSocialAccountData);
            }

        }

    }, [getPostsByBatchIdList]);


    const handleSeparateCaptionHashtag = (inputText) => {

        // Check if the inputText contains a hashtag
        const hashtagIndex = inputText.indexOf('#');

        if (hashtagIndex !== -1) {
            // Extract the caption and hashtag
            ...
            setHashTag(extractedHashtag);
        } else {
            // No hashtag found, treat the whole input as the caption
                        ...
                        <div className="row">
                            <div className="col-lg-6 col-md-12 col-sm-12">
                                ...
                                <div className="create_post_content">
                                    ...
                                    <h2 className='creare_post_heading'>{formType === "Add" ? jsondata.createpost : jsondata.updatepost}</h2>
...
export default CommonPostFields