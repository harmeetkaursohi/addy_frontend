...
const SocialAccounts = ({}) => {
  ...
  const [currentConnectedFacebookPages, setCurrentConnectedFacebookPages] =
    useState(null);
  const [currentConnectedInstagramPages, setCurrentConnectedInstagramPages] =
    useState(null);
  const [currentConnectedPinterestPages, setCurrentConnectedPinterestPages] =
    useState(null);
  const [currentConnectedLinkedinPages, setCurrentConnectedLinkedinPages] =
    useState(null);
  const [
  ...
  const openInstagramGuid = () => {
    setShowInstagramGuide(true);
  };
  const [showNoBusinessAccountModal, setShowNoBusinessAccountModal] =
    useState(false);
  const getConnectedSocialAccountApi = useGetConnectedSocialAccountQuery("");
  const connectedSocialAccount = getConnectedSocialMediaAccount(
    getConnectedSocialAccountApi?.data || []
  );

  const [connectSocialAccount, connectSocialAccountApi] =
    useConnectSocialAccountMutation();

  const [disconnectSocialAccount, disconnectSocialAccountApi] =
    useDisconnectSocialAccountMutation();
  ...
  const getAllFacebookPagesApi = useGetAllFacebookPagesQuery(
    ...
    },
    {
      skip:
        ...
        isNullOrEmpty(connectedSocialAccount.facebook),
    }
  );

  const getAllInstagramPagesApi = useGetAllInstagramBusinessAccountsQuery(
    connectedSocialAccount?.instagram?.accessToken,
    {
      skip:
        !enabledSocialMedia?.isInstagramEnabled ||
        isNullOrEmpty(connectedSocialAccount.instagram),
    }
  );

  const getAllPinterestPagesApi = useGetAllPinterestBoardsQuery(
    connectedSocialAccount?.pinterest?.id,
    {
      skip:
        !enabledSocialMedia?.isPinterestEnabled ||
        isNullOrEmpty(connectedSocialAccount.pinterest),
    }
  );
  ...
  const getAllLinkedinPagesApi = useGetAllLinkedinPagesQuery(
    ...
    },
    {
      skip:
        ...
        isNullOrEmpty(connectedSocialAccount.linkedin),
    }
  );
  ...
  useEffect(() => {
    if (
      ...
      getAllConnectedPagesApi?.data &&
      Array.isArray(getAllConnectedPagesApi?.data) &&
      !isNullOrEmpty(getAllPinterestPagesApi?.data)
    ) {
      const connectedPinterestSocialAccount =
        getConnectedSocialAccountApi?.data?.filter(
          (socialAccount) => socialAccount?.provider === "PINTEREST"
        )[0];
      const connectedPinterestBoards = getAllConnectedPagesApi?.data?.filter(
        (pageData) =>
          pageData?.socialMediaAccountId === connectedPinterestSocialAccount?.id
      );
      const currentConnectedPinterestBoards =
        getAllPinterestPagesApi?.data?.items?.filter((board) =>
          connectedPinterestBoards?.some(
            (pinBoard) => pinBoard?.pageId === board?.id
          )
        );
  ...
  useEffect(() => {
    if (
      ...
      getAllConnectedPagesApi?.data &&
      Array.isArray(getAllConnectedPagesApi?.data) &&
      !isNullOrEmpty(getAllInstagramPagesApi?.data)
    ) {
      const connectedInstagramSocialAccount =
        getConnectedSocialAccountApi?.data?.filter(
          (socialAccount) => socialAccount?.provider === "INSTAGRAM"
        )[0];
      const connectedInstagramPages = getAllConnectedPagesApi?.data?.filter(
        (pageData) =>
          pageData?.socialMediaAccountId === connectedInstagramSocialAccount?.id
      );
      const currentConnectedInstagramPages =
        getAllInstagramPagesApi?.data?.filter((page) =>
          connectedInstagramPages?.some(
            (instaPage) => instaPage?.pageId === page?.id
          )
        );
  ...
  useEffect(() => {
    if (
      ...
      const connectedFacebookSocialAccount =
        getConnectedSocialAccountApi?.data?.filter(
          (socialAccount) => socialAccount?.provider === "FACEBOOK"
        )[0];
      const connectedFacebookPages = getAllConnectedPagesApi?.data?.filter(
        (pageData) =>
          pageData?.socialMediaAccountId === connectedFacebookSocialAccount?.id
      );
      const currentConnectedFaceBookPages =
        getAllFacebookPagesApi?.data?.filter((page) =>
      ...
      .then(async (res) => {
        ...
        if (res?.socialAccountData?.name) {
          await handleRTKQuery(
            ...
            () => {
              dispatch(
                addyApi.util.invalidateTags(["getConnectedSocialAccountApi"])
              );
              dispatch(
    ...
    Swal.fire({
      ...
      html: `
               <div class="swal_content">
                ...
                <h2 class="disconnect_title">Disconnect ${getInitialLetterCap(
                  SocialAccountProvider[socialMediaType]
                )} Account</h2>
                <p class="disconnect_paragraph">${formatMessage(
                  DisconnectAccountWarning,
                  [getInitialLetterCap(SocialAccountProvider[socialMediaType])]
                )}</p>
                </div>
              `,
      ...
      if (result.isConfirmed) {
        ...
        await handleRTKQuery(
          async () => {
            const socialMediaAccountId =
              getConnectedSocialAccountApi?.data?.find(
                (c) => c.provider === socialMediaType
              )?.id;
          ...
          () => {
            ...
            dispatch(
              addyApi.util.invalidateTags(["getSocialMediaPostsByCriteriaApi"])
            );
          },
          () => {
            Swal.fire({
              ...
              title: "Error",
              confirmButtonColor: "#F07C33",
              text: `An error occurred while disconnecting your ${getInitialLetterCap(
                SocialAccountProvider[socialMediaType]
  ...
  return (
    <div className="col-lg-5 col-xl-4 col-sm-12">
          ...
          <>
            ...
            getConnectedSocialAccountApi?.isFetching ? (
              ...
              <div className="social_media_outer">
                ...
                <LoginSocialFacebook
                  isDisabled={
                    connectSocialAccountApi?.isLoading ||
                    connectSocialAccountApi?.isFetching ||
                    getConnectedSocialAccountApi?.isLoading ||
                    getConnectedSocialAccountApi?.isFetching
                  }
                  appId={`${import.meta.env.VITE_APP_FACEBOOK_CLIENT_ID}`}
                  redirect_uri={`${
                    import.meta.env.VITE_APP_OAUTH2_REDIRECT_URL
                  }/dashboard`}
                  onResolve={(response) => {
                    setFacebookDropDown(true);
                    setInstagramDropDown(false);
                  ...
                  <div className="social_media_outer cursor-pointer">
                    ...
                    >
                      <div className={"social_media_icon"}>
                        ...
                        <h2
                          className={`pagecount ${
                            currentConnectedFacebookPages?.length === undefined
                              ? "blink"
                              : ""
                          }`}
                        >
                          {currentConnectedFacebookPages !== null &&
                          currentConnectedFacebookPages !== undefined
                            ? currentConnectedFacebookPages?.length
                            : 0}
                        </h2>
                      </div>
                      ...
                      {(!getAllFacebookPagesApi?.isLoading ||
                        !getAllFacebookPagesApi?.isFetching ||
                        !getConnectedSocialAccountApi?.isLoading ||
                        getConnectedSocialAccountApi?.isFetching ||
                        ...
                        currentConnectedFacebookPages?.length === 0 && (
                          <button
                            ...
                            onClick={() =>
                              disConnectSocialMediaAccountToCustomer("FACEBOOK")
                            }
                          >
                            Disconnect
                          </button>
                    ...
                    <ul className="menu_items">
                      ...
                      getAllFacebookPagesApi?.isFetching ? (
                        ...
                        <div className={"no-page-connected-outer text-center"}>
                          ...
                          <button
                            className={
                              " cursor-pointer connect-page-btn clear_all_button_outer mt-3"
                            }
                            onClick={() => facebook()}
                          >
                            Connect now
                        ...
                        <>
                          ...
                          <li>
                            {getAllFacebookPagesApi?.data &&
                              Array.isArray(getAllFacebookPagesApi?.data) && (
                ...
                <div className="dropdown_header">
                  <div className="social_media_outer cursor-pointer">
                    ...
                    >
                      ...
                      <h2
                        className={`pagecount ${
                          currentConnectedInstagramPages?.length === undefined
                            ? "blink"
                            : ""
                        }`}
                      >
                        {currentConnectedInstagramPages !== null &&
                        currentConnectedInstagramPages !== undefined
                          ? currentConnectedInstagramPages?.length
                          : 0}
                      </h2>
                  ...
                  {instagramDropDown === true && (
                    <ul className="menu_items">
                      ...
                      getAllInstagramPagesApi?.isFetching ? (
                        <SkeletonEffect count={3} />
                      ) : currentConnectedInstagramPages?.length === 0 ? (
                        <div className={"no-page-connected-outer text-center"}>
                          <div>No active connections at the moment.</div>
            ...
            getConnectedSocialAccountApi?.isFetching ? (
              ...
              <div className="social_media_outer">
                ...
                <LoginSocialLinkedin
                  ...
                  client_secret={`${
                    import.meta.env.VITE_APP_LINKEDIN_CLIENT_SECRET
                  }`}
                  ...
                  onResolve={(response) => {
                    ...
                    connectSocialMediaAccountToCustomer(
                      computeAndSocialAccountJSON(
                        response,
                        SocialAccountProvider.LINKEDIN,
                        setShowNoBusinessAccountModal
                      ),
                      SocialAccountProvider.LINKEDIN
                    );
                ...
                <div className="dropdown_header">
                  <div className="social_media_outer cursor-pointer">
                    ...
                    >
                      ...
                      <h2
                        className={`pagecount ${
                          currentConnectedLinkedinPages?.length === undefined
                            ? "blink"
                            : ""
                        }`}
                      >
                        {currentConnectedLinkedinPages !== null
                          ? currentConnectedLinkedinPages?.length
                          : 0}
                      </h2>
                  ...
                  {linkedinDropDown && (
                    <ul className="menu_items">
                      ...
                      getAllLinkedinPagesApi?.isFetching ? (
                        <SkeletonEffect count={3} />
                      ) : currentConnectedLinkedinPages?.length === 0 ? (
                        <div className={"no-page-connected-outer text-center"}>
                          <div>No active connections at the moment.</div>
                        ...
                        <>
                          ...
                          <li>
                            {getAllLinkedinPagesApi?.data &&
                              getAllLinkedinPagesApi?.data?.results &&
                              Object.keys(getAllLinkedinPagesApi?.data?.results)
                                ?.length > 0 && (
                                <div className="connectDisconnect_btn_outer">
                                  <button
                                    className="DisConnectBtn cmn_connect_btn"
            ...
            getConnectedSocialAccountApi?.isFetching ? (
              ...
              <div className="social_media_outer">
                ...
                <LoginSocialPinterest
                  ...
                  client_secret={`${
                    import.meta.env.VITE_APP_PINTEREST_CLIENT_SECRET
                  }`}
                  ...
                  onResolve={(response) => {
                    ...
                    connectSocialMediaAccountToCustomer(
                      computeAndSocialAccountJSON(
                        response,
                        SocialAccountProvider.PINTEREST,
                        setShowNoBusinessAccountModal
                      ),
                      SocialAccountProvider.PINTEREST
                    );
                ...
                <div className="dropdown_header">
                  <div className="social_media_outer cursor-pointer">
                    ...
                    >
                      ...
                      <h2
                        className={`pagecount ${
                          currentConnectedPinterestPages?.length === undefined
                            ? "blink"
                            : ""
                        }`}
                      >
                        {currentConnectedPinterestPages !== null
                          ? currentConnectedPinterestPages?.length
                          : 0}
                      </h2>
                      <div className="text-start flex-grow-1">
                        <h5 className="userName">
                  ...
                  {pinterestDropDown === true && (
                    <ul className="menu_items">
                      ...
                      getAllPinterestPagesApi?.isFetching ? (
                        <SkeletonEffect count={3} />
                      ) : currentConnectedPinterestPages?.length === 0 ? (
                        <div className={"no-page-connected-outer text-center"}>
                          <div>No active connections at the moment.</div>
                        ...
                        <>
                          ...
                          <li>
                            {getAllPinterestPagesApi?.data &&
                              getAllPinterestPagesApi?.data?.items &&
                              Array.isArray(
                                getAllPinterestPagesApi?.data?.items
                              ) && (
                                <div className="connectDisconnect_btn_outer">
      ...
      {enabledSocialMedia?.isFacebookEnabled && showFacebookModal && (
        isNullOrEmpty(getAllFacebookPagesApi?.data || []) ? (
      ...
      {enabledSocialMedia?.isLinkedinEnabled && showLinkedinModal && (
        <ConnectPagesModal
          ...
          allPagesList={Object.keys(
            getAllLinkedinPagesApi?.data?.results || {}
          )?.map((key) => {
            return getFormattedLinkedinObject(
              key,
              getAllLinkedinPagesApi?.data?.results[key]
            );
          })}
          connectedPagesList={getAllConnectedPagesApi?.data}
          noPageFoundMessage={"No Page Found, Please connect another account."}
